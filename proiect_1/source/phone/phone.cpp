...
void Phone::displayPhone(){
     ...
     std::cout << phoneType << " " << color << " " <<proccesor << std::endl;
}

PhoneTypes Phone::getPhoneType() const{
     return this->phone;
}

colorTypes Phone::getPhoneColor() const{
     return this->color;
}

proccesorTypes Phone::getPhoneProccesor() const{
...
Phone& Phone::operator=(const Phone& cPhone){
     if(this == &cPhone) return *this;

     battery = cPhone.battery;
     ram = cPhone.ram;
     internalStorage = cPhone.internalStorage;
     g5 = cPhone.g5;
     wifi = cPhone.wifi;
     bluetooth = cPhone.bluetooth;
     dualSim = cPhone.dualSim;
     waterResistance = cPhone.waterResistance;
     phone = cPhone.phone;
     color = cPhone.color;
     proccesor = cPhone.proccesor;
